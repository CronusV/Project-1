...
router.post('/', validateMW.validateUser, async (req, res) => {
  const body = req.body;
  ...
  console.log(`this is validateTicket obj: ${JSON.stringify(validateTicket)}`);
  ...
  if (picture) {
    ...
    try {
      ...
      res.status(200).send({ message: 'Successfully added ticket with image' });
    } catch (err) {
